...
#
# $Id: erl.c 1539 2009-03-11 03:22:39Z jim $
...
return code
   ...
   // Reading the main ELF header.
	...
	memcpy(&head, elf_mem, sizeof(head));
        ...
        fseek(elf_handle, 0, SEEK_SET);
        fread(&head, 1, sizeof(head), elf_handle);
    }
   ...
   // Reading the section table.
	...
	fseek(elf_handle, head.e_shoff, SEEK_SET);
        fread(sec, 1, sizeof(struct elf_section_t) * head.e_shnum, elf_handle);
    }
   ...
   // Reading the section names's table.
	...
	if (!(names = (char *) malloc(sec[head.e_shstrndx].sh_size))) {
        ...
        fseek(elf_handle, sec[head.e_shstrndx].sh_offset, SEEK_SET);
        fread(names, 1, sec[head.e_shstrndx].sh_size, elf_handle);
    }
   ...
   // Loading progbits sections.
	...
	case PROGBITS:
		...
		fseek(elf_handle, sec[i].sh_offset, SEEK_SET);
		fread(erl_record->bytes + sec[i].sh_addr, 1, sec[i].sh_size, elf_handle);
	    }
	    break;
	case NOBITS:
   ...
   // Loading strtab.
	...
	fseek(elf_handle, sec[strtab].sh_offset, SEEK_SET);
	fread(strtab_names, 1, sec[strtab].sh_size, elf_handle);
    }
   ...
   // Loading symtab.
	...
	if (!(sym = (struct elf_symbol_t *) malloc(sec[symtab].sh_size))) {
        ...
        fseek(elf_handle, sec[symtab].sh_offset, SEEK_SET);
	    fread(sym, 1, sec[symtab].sh_size, elf_handle);
    }
	...
	if (sec[i].sh_entsize != sizeof(struct elf_reloc_t)) {
       ...
       // Loading relocation section.
	    ...
	    if (!(reloc_section = (char *) malloc(sec[i].sh_size))) {
            ...
            fseek(elf_handle, sec[i].sh_offset, SEEK_SET);
            fread(reloc_section, 1, sec[i].sh_size, elf_handle);
        }
...
static struct erl_record_t * load_erl(const char * fname, u8 * elf_mem, u32 addr, int argc, char ** argv) {
    ...
    if (fname) {
        if ((elf_handle = fopen(fname, "rb")) < 0) {
