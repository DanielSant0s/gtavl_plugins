...
CVector getVehicleDummyPos(uint16_t veh_id, uint16_t dummy_id) {
	CVector ret;
...
static void DrawVehicleReverselights(uint32_t vehicle) {
	CVector posn = getVehicleDummyPos(*(uint16_t*)(vehicle + 0x22), 1);
...
void (*CVector_Normalise)(CVector*) = (void (*)(CVector*))0x110390;
...
void CVehicle_DoSunGlare(int this)
{
      ...
      && CWeather_SunGlare > 0.0f )
    {
        ...
        r = ((float)(CTimeCycle_m_CurrentColours->m_nSunCoreRed + 510)
                                * intensity
                                * 0.33333334f);
        g = ((float)(CTimeCycle_m_CurrentColours->m_nSunCoreGreen + 510)
                                * intensity
                                * 0.33333334f);
        b = ((float)(CTimeCycle_m_CurrentColours->m_nSunCoreBlue + 510)
                                * intensity
                                * 0.33333334f);
...
void hookedVehicleRender(uint32_t vehicle) {
	...
	(((union VehFlags)getVehicleFlags(vehicle)).data).bEngineOn && !*(char*)(0x66B310) && getVehicleHealth(vehicle) > 0.0f && !(((union VehFlags)getVehicleFlags(vehicle)).data).bIsDrowning && !attachedTo(vehicle))
	{
...
int hookedExitVehicle(int a1) {
	...
	if(ret) {
		CVehicle__SetEngineState(FindPlayerVehicle(-1, 0), 0);
	}
	return ret;
}
...
void setupVehiclePatches() {
	...
	RedirectCall(0x15F95C, hookedVehicleRender);
	RedirectCall(0x2B4AC0, hookedVehicleRender);
	RedirectCall(0x31D1D8, hookedVehicleRender);
	RedirectCall(0x32B998, hookedVehicleRender);
