...
void rotate_clut() {
    ...
    if (CTimer_m_snTimeInMilliseconds > time_to_rotate) {
        ...
        if (shift_count > 14) {
            ...
            shift_count = 0;
            hold = true;
            hold_coef = CTimer_m_snTimeInMilliseconds + (35*randint(1, 6));
        }

        time_to_rotate = CTimer_m_snTimeInMilliseconds + (35*randint(1, 2));
    }
...
bool SkyEdgeCameraClear(uint8_t *pCam, RwRGBA *col, int nClearWhich)
{
    ...
    _rwDMAOpenVIFPkt(RWDMA_FIXUP, 16);

	// Clear
    tmp = GIF_SET_TAG(5, 1, GIF_PRE_ENABLE, GS_PRIM_SPRITE, GIF_PACKED, 3);
    tmp1 = GIF_AD | (GIF_AD << 4) | (GIF_AD << 8);
    ADDTOPKT(tmp1, tmp);

    //---------------------------------------------------------//

    tmp = GS_SET_RGBAQ(0x7F, 0x00, 0x00, 0x7F, 1);
    tmp1 = GS_RGBAQ;
    ADDTOPKT(tmp1, tmp);
    ...
    tmp = GS_SET_ZBUF_1(skyFrame_1, GS_PSMZ32, 0);
    tmp1 = GS_ZBUF_1;
    ADDTOPKT(tmp1, tmp);
    ...
    tmp = GS_SET_XYZ2((32 << 4), 2048 << 4, 0x6000007F);
    tmp1 = GS_XYZ2;
    ADDTOPKT(tmp1, tmp);

    //---------------------------------------------------------//

    tmp = GS_SET_RGBAQ(0x00, 0x7F, 0x00, 0x7F, 1);
    tmp1 = GS_RGBAQ;
    ADDTOPKT(tmp1, tmp);

    tmp = GS_SET_FRAME_1((skyFrame_1) + 64, 1, GS_PSMCT32, 0);
    tmp1 = GS_FRAME_1;
    ADDTOPKT(tmp1, tmp);

    tmp = GS_SET_ZBUF_1((skyFrame_1) + 64, GS_PSMZ32, 0);
    tmp1 = GS_ZBUF_1;
    ADDTOPKT(tmp1, tmp);
    ...
    tmp = GS_SET_XYZ2((32 << 4), 2048 << 4, 0x60007F00);
    tmp1 = GS_XYZ2;
    ADDTOPKT(tmp1, tmp);

    //---------------------------------------------------------//

    tmp = GS_SET_RGBAQ(0x00, 0x00, 0x7F, 0x7F, 1);
    tmp1 = GS_RGBAQ;
    ADDTOPKT(tmp1, tmp);

    tmp = GS_SET_FRAME_1((skyFrame_1) + 128, 1, GS_PSMCT32, 0);
    tmp1 = GS_FRAME_1;
    ADDTOPKT(tmp1, tmp);

    tmp = GS_SET_ZBUF_1((skyFrame_1) + 128, GS_PSMZ32, 0);
    tmp1 = GS_ZBUF_1;
    ADDTOPKT(tmp1, tmp);
    ...
    tmp = GS_SET_XYZ2((32 << 4), 384 << 4, 0x607F0000);
    tmp1 = GS_XYZ2;
    ADDTOPKT(tmp1, tmp);
