...
static WORD& DecLogicalOperation(WORD& o)
{
    if (o == NONE) return o; // can not be decremented anymore
    if (o == OR_2) return o = NONE;

    WORD val = static_cast<WORD>(o); // to number
    val--;
...
public:
    ...
    inline	float			 ReadDataFloat()
    {
        float f = *(float*)CurrentIP;
        CurrentIP += 4;
        return f;
    };
    ...
    inline WORD GetScmFunction() { return scmFunction; }
    inline void SetScmFunction(WORD id) { scmFunction = id; }
