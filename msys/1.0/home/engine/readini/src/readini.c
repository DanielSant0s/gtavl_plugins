...
bool readini_open(IniReader* ini, const char* fname) {
    ...
    ini->handle = fs->rwfopen(fname, "r");
    return ini->handle;
}

bool readini_close(IniReader* ini) {
    RwFileFunction *fs = RwOsGetFileInterface(); 
    return fs->rwfclose(ini->handle);
}
...
bool readini_getline(IniReader* ini) {
    ...
    if( !fs->rwfgets(ini->cur_line, LINE_BUFSIZE, ini->handle) ) {
        if(!fs->rwfeof(ini->handle)) {
            ini->cur_line[0] = '\0';
            return true;
        }
        return false;
...
bool readini_float(IniReader* ini, const char* key, float* value_ptr) {
    ...
    if ( !ret )
    {
        *value_ptr = dptofp(atof(value_str));
    }
