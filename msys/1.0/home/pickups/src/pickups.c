...
static void (*updatePickup)(uint32_t pickup, CPlayerPed *player, CVehicle *vehicle, int playerId) = (void (*)(uint32_t pickup, CPlayerPed *player, CVehicle *vehicle, int playerId))0x2D9D50;
static uint32_t** ms_modelInfoPtrs = (uint32_t**)0x7A8780;
static void (*GetBoundCentre)(uint32_t, CVector*) = (void (*)(uint32_t, CVector*))0x22F3B0;
...
void pickups_on_ground(uint32_t pickup, CPlayerPed *player, CVehicle *vehicle, int playerId) {
    ...
    if (pickup_type == 2 || pickup_type == 3 || pickup_type == 4 || pickup_type == 5 ||
        ...
        if (x != 0 && pObject != 0) {
            ...
            tmp.y = player_coords.y - coords->y;ss
