...
# Review ps2sdk rw_readME & LICENSE files for further details.
*/
...
static struct erl_record_t **erl_record_root = (struct erl_record_t **)0x3940A8;

static htab **loosy_relocs = (htab **)0x3940B0;

static htab **global_symbols = (htab **)0x3940AC;
static struct dependancy_t **dependancy_root = (struct dependancy_t **)0x3940B4;
static reroot **symbol_recycle = (reroot **)0x3940B8;

static FILE *f = (FILE *)NULL;
static RwFileFunction* fs = (RwFileFunction *)NULL;

void open_symtable_file() {
    f = rw_open("symbols.dat", "r");
    fs = RwOsGetFileInterface();
}

void close_symtable_file() {
    rw_close(f);
}

unsigned int find_symtable_file(const char* sym_name) {
    char line[256];
    char symName[256];
    unsigned int symAddress, ret = 0;

    fs->rwfseek(f, 0, SEEK_SET);
    ...
    while (fs->rwfgets(line, sizeof(line), f) != NULL) {
        ...
        if (sscanf(line, "%255s %x", symName, &symAddress) == 2) {
...
struct symbol_t * erl_find_local_symbol(const char * symbol, struct erl_record_t * erl) {
    if (!erl) return 0;
    if (hfind(erl->symbols, symbol, strlen(symbol))) return hstuff(erl->symbols);
    return 0;
}
...
struct symbol_t * erl_find_symbol(const char * symbol) {
    if (*global_symbols) if (hfind(*global_symbols, symbol, strlen(symbol))) return hstuff(*global_symbols);
...
static int read_erl(FILE* elf_handle, u8 * elf_mem, u32 addr, struct erl_record_t ** p_erl_record) {
...
return code
   ...
   // rw_reading the main ELF header.
    if (elf_mem) {
	    memcpy(&head, elf_mem, sizeof(head));
    } else {
        rw_seek(elf_handle, 0, SEEK_SET);
        rw_read(elf_handle, &head, sizeof(head));
    }
   ...
   // rw_reading the section table.
    ...
    if (elf_mem) {
	    ...
	    rw_seek(elf_handle, head.e_shoff, SEEK_SET);
        rw_read(elf_handle, sec, sizeof(struct elf_section_t) * head.e_shnum);
    }

   // rw_reading the section names's table.
    // **TODO** handle compession
    if (elf_mem) {
	...
	if (!(names = (char *) malloc(sec[head.e_shstrndx].sh_size))) {
        ...
        rw_seek(elf_handle, sec[head.e_shstrndx].sh_offset, SEEK_SET);
	    rw_read(elf_handle, names, sec[head.e_shstrndx].sh_size);
    }
   ...
   // Loading progbits sections.
	...
	case PROGBITS:
            // **TODO** handle compession
	    dprintf("rw_reading section %s at %08X.\n", names + sec[i].sh_name, erl_record->bytes + sec[i].sh_addr);
	    if (elf_mem) {
		    memcpy(erl_record->bytes + sec[i].sh_addr, elf_mem + sec[i].sh_offset, sec[i].sh_size);
	    } else {
		    rw_seek(elf_handle, sec[i].sh_offset, SEEK_SET);
		    rw_read(elf_handle, erl_record->bytes + sec[i].sh_addr, sec[i].sh_size);
	    }
	    break;
	case NOBITS:
   ...
   // Loading strtab.
    ...
    if (elf_mem) {
	    ...
	    rw_seek(elf_handle, sec[strtab].sh_offset, SEEK_SET);
	    rw_read(elf_handle, strtab_names, sec[strtab].sh_size);
    }
   ...
   // Loading symtab.
	...
	if (!(sym = (struct elf_symbol_t *) malloc(sec[symtab].sh_size))) {
        ...
        rw_seek(elf_handle, sec[symtab].sh_offset, SEEK_SET);
	    rw_read(elf_handle, sym, sec[symtab].sh_size);
    }
   ...
   // Parsing sections to find relocation sections.
    for (i = 0; i < head.e_shnum; i++) {
       ...
       // Loading relocation section.
	    ...
	    if (!(reloc_section = (char *) malloc(sec[i].sh_size))) {
            ...
            rw_seek(elf_handle, sec[i].sh_offset, SEEK_SET);
	        rw_read(elf_handle, reloc_section, sec[i].sh_size);
        }
	...
	for (j = 0; (u32)j < (sec[i].sh_size / sec[i].sh_entsize); j++) {
	    ...
	    case NOTYPE:
            ...
            if ((symbol_address = find_symtable_file(strtab_names + sym[sym_n].st_name))) {
                printf("Found symbol at symbol table file. Address: %08X\n", symbol_address);
                if (apply_reloc(erl_record->bytes + sec[sec[i].sh_info].sh_addr + reloc.r_offset, reloc.r_info & 255, symbol_address) < 0) {
		    	    printf("Something went wrong in relocation.");
		    	    free_and_return(-1);
		        }
            } else if (!(s = erl_find_symbol(strtab_names + sym[sym_n].st_name))) {
	    ...
	    case FUNC:
            ...
            if ((symbol_address = find_symtable_file(strtab_names + sym[sym_n].st_name))) {
		        ...
		        dprintf("Symbol alrw_ready exists at %08X. Let's use it instead.\n", s->address);
		        if (apply_reloc(erl_record->bytes + sec[sec[i].sh_info].sh_addr + reloc.r_offset, reloc.r_info & 255, s->address) < 0) {
		    	    dprintf("Something went wrong in relocation.");
		    	    free_and_return(-1);
		        }
	...
	if (((sym[i].st_info >> 4) == GLOBAL) || ((sym[i].st_info >> 4) == WEAK)) {
		...
		if (add_symbol(erl_record, strtab_names + sym[i].st_name, ((u32)erl_record->bytes) + sec[sym[i].st_shndx].sh_addr + sym[i].st_value) < 0) {
		    dprintf("Symbol probably alrw_ready exists, let's ignore that.\n");
//		    free_and_return(-1);
		}
	    }
	}
...
static struct erl_record_t * _init_load_erl_wrapper_from_file(char * erl_id) {
    ...
    strcat(tmpnam, ".erl");
    return load_plugin(tmpnam, erl_id);
}
...
static struct erl_record_t * load_erl(const char * fname, u8 * elf_mem, u32 addr, int argc, char ** argv) {
    ...
    if (fname) {
        if ((elf_handle = rw_open(fname, "rb")) < 0) {
    ...
    dprintf("Total memory allocated for plugins: %d bytes\n", *PluginUsedMemory);
...
struct erl_record_t * load_plugin(const char * fname, char * erl_id) {
    ...
    return load_erl(fname, 0, ERL_DYN_ADDR, 1, argv);
}
